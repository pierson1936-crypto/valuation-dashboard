...
- ETF/指数：股价近 5 年历史分位、技术指标、资金流和规则报告；当前不提供
  ETF/指数级 PE/PB。
- 大盘：主要指数行情、同花顺行业资金净额排序和 SVG 动效；同花顺失败时回退东方财富，双源延迟时显示最近一次完整快照。
...
面向普通使用者：双击 `启动.bat`。它会启动服务并自动使用 Microsoft Edge 打开页面；
未找到 Edge 时才回退到系统默认浏览器。
...
- 东方财富：代码识别、PE/PB、行业、基本面、个股资金流和行业板块主力净流入；
- 腾讯证券：前复权日 K、实时报价和大盘指数行情；
- 新浪财经：腾讯日 K 不可用或样本过少时的备用来源。

个股完整分析在进程内缓存 180 秒，大盘概览缓存 120 秒，行业资金流缓存 300 秒。
行业资金流先显示最近快照再在后台更新；更新失败不会清空上一份完整榜单，
页面会明确标注数据时间和“最近完整快照”。进程重启会清空内存缓存，但不删除该运行时快照。
