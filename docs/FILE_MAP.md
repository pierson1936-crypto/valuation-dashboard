...
| `monitoring/web.py` | Web 脱敏总览、三线设置、提醒预览和后台启停 | `app.Handler` | 是 | 中 | 模拟预览不得取行情、写事件、发通知或调用模型 |
...
| `monitoring/explanations.py` | 按需事件复核、严格 JSON、缓存和每日额度 | Web 盯盘 | 否 | 中 | 只读事件和逻辑卡，不进入分钟轮询 |
...
| `tests/test_analysis.py` | 分位口径和完整分析契约 | unittest | 是 | 低 | 核心计算改动的最低保护 |
| `tests/test_http.py` | 随机端口 HTTP/API 冒烟测试 | unittest | 是 | 低 | Mock 业务与模型调用 |
...
| 标的/行情 | `_guess`、`resolve`、`fetch_kline`、`fetch_quote` |
| 估值/财务/资金 | `fetch_valuation`、`fetch_industry`、`fetch_fundamentals`、`fetch_moneyflow` |
| 纯计算 | `sma`、`ema`、`rsi`、`macd`、`boll`、`percentile_rank`、`stat_block` |
| 综合分析 | `analyze`、`build_report`、`build_alerts`、`analyze_cached` |
| Excel | `build_excel` |
| AI | `AGENT_TOOLS`、`agent_run`、`analyze_multidim`、`panel_analyze` |
| 前端 | `HTML` |
| HTTP/启动 | `Handler`、`if __name__ == "__main__"` |
